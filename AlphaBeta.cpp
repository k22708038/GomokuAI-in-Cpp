...
int alive3_cnt = 0, death4_cnt = 0; // for bonus shape combo
...
bool OutstandingMove(int x, int y, int Chess_Player) {
    int serial_cnt = 1;
    for (int dx = x + 1; dx <= x + 4; dx++) {
        if (in_board(dx, y)) {
            if (board[dx][y] != Chess_Player) {
                break;
            }
            serial_cnt++;
        } else {
    ...
    for (int i = x + 1; i <= x + 4; i++) {
        if (in_board(i, i - diff)) {
            if (board[i][i - diff] != Chess_Player) {
                break;
            }
            serial_cnt++;
        } else {
...
int Shape_to_Score(int serial_cnt, bool SINGLE_LIVE, bool DOUBLE_LIVE,
                   bool DOUBLE_DEATH) {
    int score = 0;
    if (serial_cnt == 1) {
        if (DOUBLE_LIVE) {
    ...
    for (int i = 0; i < SIZE; i++) {
        ...
        for (int j = 0; j < SIZE; j++) {
            if (board[i][j] == Chess_player) {
                ...
                score +=
                    Shape_to_Score(serial_cnt, SINGLE_LIVE, DOUBLE_LIVE, DOUBLE_DEATH);
                serial_cnt = 0;
                SINGLE_LIVE = false;
                DOUBLE_LIVE = false;
                DOUBLE_DEATH = false;
        ...
        for (int i = 0; i <= sum; i++) {
            if (in_board(i, sum - i)) {
                if (board[i][sum - i] == Chess_player) {
                    ...
                    score += Shape_to_Score(serial_cnt, SINGLE_LIVE, DOUBLE_LIVE,
                                            DOUBLE_DEATH);
                    serial_cnt = 0;
                    SINGLE_LIVE = false;
                    DOUBLE_LIVE = false;
...
int AlphaBeta(int maximizingPlayer, int depth, int alpha, int beta, std::ofstream &fout) {
    using namespace std;
    int value;
    if (depth == 0) {
        return Calculate_Score(player) - Calculate_Score(opponent) * 1.1;
    }
    if (maximizingPlayer == player) {
        ...
        for (int i = 0; i < SIZE; i++) {
            for (int j = 0; j < SIZE; j++) {
                if (!empty_around(i, j) && board[i][j] == EMPTY) {
                    ...
                    int score = AlphaBeta(opponent, depth - 1, alpha, beta, fout);
                    board[i][j] = EMPTY;
                    if (score > value) {
                        value = score;
                        if (depth == DEPTH) {
        ...
        for (int i = 0; i < SIZE; i++) {
            for (int j = 0; j < SIZE; j++) {
                if (!empty_around(i, j) && board[i][j] == EMPTY) {
                    if (OutstandingMove(i, j, opponent)) {
                        return -WIN5;
                    }
                    board[i][j] = opponent;
                    int score = AlphaBeta(player, depth - 1, alpha, beta, fout);
                    board[i][j] = EMPTY;
...
int main(int, char **argv) {
    ...
    if (first_hand) {
        ...
        AlphaBeta(player, DEPTH, INT_MIN, INT_MAX, fout);
    }
    fin.close();
    fout.close();
    return 0;
